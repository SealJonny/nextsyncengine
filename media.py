...
class Extract:
    ...
    def set_exiftool(self, exiftool):
        if not os.path.isfile(exiftool):
            return FileNotFoundError(f"Setting exiftool path failed! There is not file at {exiftool}")
        self.exiftool = exiftool
        return None

    def __get_datetime_media(self, media_path):
        """
        ...
        ctime_cmd = f"{self.exiftool} -m -s3 -d '%Y:%m:%d %H:%M:%S' -DateTimeOriginal -DateCreated -CreateDate -FileCreateDate '{media_path}' | head -n 1"
        mtime_cmd = f"{self.exiftool} -m -s3 -d '%Y:%m:%d %H:%M:%S' -DateTime -ModifyDate -FileModifyDate '{media_path}' | head -n 1"
        ...
        if ctime_str == '':
            return self.__get_datetime_non_media(media_path)
    ...
    def __get_datetime_non_media(self, video_path):
        """
    ...
    def get_datetime(self, media_path):
        """
        ...
        for f in SUPPORTED_FORMATS:
            if ext == f:
                return self.__get_datetime_media(media_path)
        return self.__get_datetime_non_media(media_path)
